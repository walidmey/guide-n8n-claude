...
{% hint style="info" %}
Cette sous-partie vous guide dans la construction d’un premier workflow IA complet avec n8n et Claude. Vous allez créer un enchaînement simple qui prend un texte, le résume automatiquement, puis envoie le résultat par email.
{% endhint %}
...
**Model :** `claude-sonnet-4-5` **Temperature :** 0.3 (pour un résumé factuel, on veut peu de créativité)
{% endstep %}
...
{% hint style="success" %}
À la fin de cette sous-partie, vous aurez un workflow n8n opérationnel capable de recevoir un texte, demander à Claude un résumé structuré, puis transmettre automatiquement ce résumé par email.
{% endhint %}
